...
**Every open entry carries a `Trigger:` line, from 15 September (seventeenth
session): the event that makes it actionable, the Order commit it waits on,
or the pending decision it belongs to.** A session reads the entries whose
...
**Trigger:** a prompt change whose predicted effect is on `measure`, `group_by`, `tickers` or `status`, which the runner cannot see; widening its fields is then the decision.
...
**Trigger:** a golden run printing `retries: N`, the first live observation of the repair path; Order 5 retires the router before or after.
...
**Trigger:** Order 6, the real portfolio.
...
**Trigger:** Order 5, the conversational layer.
...
**Trigger:** Order 6, the real portfolio: the seed is the only writer of `asset_class` and `sector`.
...
**Trigger:** Order 4, the commit that makes 4.3 answerable.
...
**Trigger:** pending decision 10, a window return: the first period that is not a year multiple.
...
**Trigger:** Order 6, the real portfolio: a tenth holding.
...
**Trigger:** pending decision 22, the return series the covariance runs on.
...
**Trigger:** `config.features.observability_enabled` set true, or Order 5.
...
**Trigger:** the line moving on a golden run.
...
**Trigger:** the next change to `load_portfolio_context`.
...
**Trigger:** pending decision 52, the Yahoo-fed tables.
...
**Trigger:** a second price source: D20 says the answer then reports both.
...
**Trigger:** a warning of a new kind on a green run.
...
### `shares_history` has no `source` column at all - RESOLVED 17 September (twenty-second session)

**Trigger:** none: the count the block carries is a filed fact with its tag, accession and filed date (730fd2d), and `shares_history` is not read; what becomes of the table is decision 52's.
...
**Resolved 17 September (twenty-second session), decision 48 item 7.**
`shares_outstanding` is a field of the block, `CommonStockSharesOutstanding`
at the fiscal year end, each count carrying the filing it came from (Part
12 B's and Part 13 B's notes); nothing reads `shares_history` for it, and
the table stays a Yahoo-fed one under decision 52.
...
**Trigger:** a pure module imported where no database exists: a fresh clone, or a tool process.
...
**Trigger:** Order 6, the personal policy.
...
**Trigger:** Order 6: before the checker grows its first personal clause type.
...
### The golden loop discards the text of a transient error

**Trigger:** the next golden run whose diff is off the pinned line and does not reproduce on a second run.

Logged 17 September (twenty-second session). The first golden run of the
session came back with its first line, "What is the current market
regime?", at intent None and one error, and the second run, eighty seconds
later, zero diff on seventeen lines; one CLI call on the same question in
between routed it as expected. The router call itself failed once, and
what it said is gone: the loop as CLAUDE.md and the handoff write it sends
stderr to /dev/null, so a transient error leaves a diff and no reason. A
line that only holds most of the time has failed, and this one held on the
second run, but the record cannot say what it was. Not a repair to make
...
node needs, in order: the block's `source`, which the module requires and
the reader's block does not carry (the node keeps the provider's name
beside the block, not on it); the five assumptions gathered into one
mapping with their sources, the three on PHI-4.1's parameters and the pair
on the watchlist entry, for which no loader exists (`watchlist.toml` is
read by its test alone); the range record published as a summary, two
floats with the year, its dates, the source and the assumptions, and no
filed figure; and, for PHI-4.1's finding and case 4.2's "price and its
as-of date", a price, which is the next entry's decision. The screen's
margin of safety reads `low` and `as_of` from the record and nothing else
of it today. A candidate that states no growth pair, W-2, must stop with
...
close the system stores belongs to an assets row of a held portfolio, so a
candidate on the watchlist has none. The shape: the last close of the
ticker the question named, on the day it is asked, stored with its source
and its date the way Part 9 defended the holdings' closes, and a Part 9
row for one candidate before any close of one is trusted. Choices the
owner has: the existing price provider through a row that is not a holding
(the `assets` table is a portfolio's, and a watchlist company is its EDGAR
number, `database_setup.py`); the exchange's own quotes Part 9 used by hand;
a price the question carries, which puts a number in the asker's hands
and is refused. Alphabet has four tickers under one CIK and the count
covers every class, so which ticker's close is the item 7 second-half
...
source, the price with its as-of date, no forecast. None of that says the
ends are right; a range of 1.00 to 2.00 per share with five assumptions
named would pass. What holds the ends is `tests/test_valuation.py` against
Part 11 C, and the seam between them is the node's assembly of the block
...
**Trigger:** the first filer whose `CommonStockSharesOutstanding` carries a unit other than `shares`.

Logged 17 September (twenty-second session), 730fd2d. The block's
one-currency rule reads every unit but `pure` and `shares` as money, a
stated set of two. Both filers here file the count in `shares`, the
taxonomy's unit for it; a filer filing it under another string would
raise the block as filed in two currencies, naming the strings. A true
...
*19 September (twenty-seventh session), the Item 7 trigger fired.* The
first live reading of Item 7, refused on the cap and read by hand, quoted
Cloud's operating income as "increased $7.8 billion from 2024 to 2025",
a change and not the year's level. W-1.2 is still settled by hand against
the segment note in Item 8, which is not read. The date trigger stands.
...
*19 September (twenty-seventh session), the 4.4 trigger fired.* 4.4 is
answerable since f5f855c and passes on the runner. The first live
readings were read by hand against the 10-K ("The live readings of Items
1A and 7, beside Item 1's"), and a live answer line by line; what the
check could not see there has its own entries ("The first live proposals
cite claims that do not support them", "A field `check_4_4` finds on
another line"). The 4.3 trigger stands.
...
*Resolved for 4.4, 19 September (twenty-seventh session).* Sighted
twice more alone, both out_of_scope; the golden line written at
out_of_scope (e9b6022); the routing commit's prediction held on all
twenty lines on two identical runs (f5f855c, 90486b5). 4.3's sighting
stands for its own routing commit.
...
*19 September (twenty-seventh session), both triggers fired.* Two new
prompt versions read Item 1 again, each accepted and stored and each
read by hand: the same three faults recurred ("The live readings of
Items 1A and 7, beside Item 1's"). The version the node serves, and the
live answer printed, is 068134c's. This row is kept and read by
nothing.
...
*19 September (twenty-seventh session), both triggers fired.* Item 1 has
been read three times under three prompt versions, and the three
readings differ claim by claim while making the same three mistakes.
Items 1A and 7 were refused differently from request to request (cap,
...
*Resolved 19 September (twenty-seventh session).* Two more sightings
...
### Item 7 under the new cap: accepted on the first request, read by hand

**Trigger:** the next prompt version for Item 7; the next proposal, which can now cite claims about margins.

Logged 20 September (twenty-eighth session), one request asked for and
said first, made after decision 70 landed. `claude-sonnet-5`, prompt
version `54b0dba223f4`, 068134c's wording unchanged, twelve claims,
**2,554 characters of quotation together against the cap of 3,600**,
accepted and stored: `document_readings` 4 rows to 5. **7.11 is the
liquidity sentence of 330 characters that was refused twice under the old
per-quote cap**; under it this whole reading would have been refused and
the other eleven claims lost with it. A thesis answer now reads all three
sections from the cache and a paid loop asks only for the proposal, about
$0.007. The request's token counts were not captured, the script keeping
the claims and not the usage, so its cost is the earlier measurement's
estimate of about $0.051 and not a measurement: a miss.

Read by hand against the stored section, each quote found once, with its
paragraph. Eight faithful: 7.1, 7.2, 7.3, 7.5, 7.7, 7.8, 7.10 and 7.11.
Three faithful at the core with something the quote does not carry: 7.4,
marked `inferred`, whose quote is the table row "Operating margin 32 % 32
% 0 %", the flat margin in the row and which column is which year nowhere
in the stored text (Part 16 E); 7.6, adding "to support AI growth" where
the quote says servers, network equipment and data centers; 7.12, adding
"significant" to a debt issuance the quote gives no figure for. **One
gets the filing wrong: 7.9 calls Waymo's charge "stock-based" where the
filing says "a valuation-based compensation charge related to Waymo".**
The twenty-seventh session's Item 7 answer made the same substitution, on
a different draw under a different prompt version, so it is a habit on
that paragraph and not one bad draw. Nothing of the prompt is changed for
it; that is a hypothesis of its own.
...
Logged 19 September (twenty-seventh session). Opened as decision 70 on
the owner's word. Part 15 D47 caps a quote at 300 characters and refuses
the whole reading when one claim breaks it. Items 1A and 7 of Alphabet's
FY2025 10-K were asked for seven times on their own in the session, under
three wordings of the prompt, and refused every time: on the cap six
times and on a quote across a page break once; the CLI's thesis question
afterwards refused them on a page break and on a digit in a claim. A
diagnostic with no request, on the stored text and the four raw answers
still kept, separated two causes. The filing writes long sentences: 64 of Item 1A's
444 sentences are over 300 characters, the longest 1,150, against 15 of
Item 7's 484 and 9 of Item 1's 174. And the model quotes on sentence
boundaries whatever limit it is told: of six quotes over the cap, three
were one whole sentence longer than 300 (341, and Item 7's liquidity
sentence at 330, twice), two were two whole sentences joined (308, 301),
one was cut from inside a sentence of 342 and still ran 326. A claim that
rests on such a sentence cannot be quoted whole under D47. What the
decision chooses between, none recommended yet: the prompt telling the
model it may quote a shorter run of words from inside a sentence, a new
hypothesis with its own prediction; D47's cap measured against this
...
**Put to it the same day**: one request, accepted on the first draw, the
330-character sentence quoted whole, in its own entry above.

### The live readings of Items 1A and 7, beside Item 1's

**Trigger:** pending decision 70; the next prompt version for any section.

Logged 19 September (twenty-seventh session). Every request to
`claude-sonnet-5`, no temperature, the answer's text kept for all but the
first. Under the prompt of a9412ea, stating 300 characters: Item 1A
`req_011CfC81qR3bXRzSXwKiGQqC`, 24,780 tokens in, 1,628 out, $0.0658,
refused on a quote of 342, the answer not kept; Item 1A
`req_011CfC86ULRJJPSYzujb555d`, $0.0673, refused on 303 and holding 321;
Item 7 `req_011CfC8E2B2mvERqMCVYWnax`, 17,660 in, 1,569 out, $0.0510,
refused on 301 and holding 330. Under c953d04, 250 characters: Item 1
held, accepted, stored; Item 1A missed, 308 and 341; Item 7 missed, 301
and 330; thirteen quotes of thirty-six over 250. Under 068134c, thirty
words: Item 1 held, accepted, stored; Item 1A missed, 326, and refused on
a quote across a page break, Part 16's F7 met live; Item 7 missed, 330;
fourteen quotes of thirty-five over thirty words. Two misses on each of
Items 1A and 7 stopped the hypothesis, and no third wording was tried. The
prompt stays at 068134c's.

Read by hand against the stored section. Item 1A under the first prompt,
the second request: three claims of twelve faithful to their quote, three
faithful at the core with a detail from the paragraph, five saying more
than their quote under `stated` (1A.10's quote is only "International
revenues accounted for approximately 52%" for a claim about currency and
geopolitics), one getting the filing wrong (the search remedies called
"structural"); all twelve `stated`; one a claim about the share's price,
the filing's own risk sentence. Item 7 under the first prompt: nine of
twelve faithful, one saying more, one saying more of a table row with no
year on it, one naming Waymo's "valuation-based" charge "stock-based";
all `stated`. Item 1 under the second and the third prompt: seven of
twelve faithful and five of twelve, the same three faults as the first
reading each time (app sales and devices beside a quote holding only
subscriptions; the list after a colon; "two" segments, a number in words).
Items read in the order of their faithfulness: Item 7, Item 1, Item 1A.
...
Logged 19 September (twenty-seventh session). A refused reading leaves no
row (D57), so since the routing commit, f5f855c, every golden run, runner
run and CLI question on a thesis asks for Items 1A and 7 again and
proposes again, about $0.12 a run by the measured requests; the golden
script and the runner record none of it. The refusal is a new draw each
time: the CLI's run refused Item 1A on a quote across a page break and
Item 7 on a digit in a claim, where the runs before refused both on the
...
**20 September (twenty-eighth session).** The entry is spent: all three
sections are cached. Item 1A was stored by one of the paid loops and Item
7 by the one request made after decision 70 landed, so a run asks for no
reading and proposes once, about $0.007, down from about $0.12. What
...
Logged 19 September (twenty-seventh session). Three live proposals, the
first on its own (`req_011CfCrVSL9t6qqvuxKaCcYy`, 2,424 tokens in, 214 out,
$0.0070), the others inside the runner's and the CLI's 4.4, all over Item
1's claims alone. Each chose `gross_margin`, `min`; the frame wrote W-1.3,
at least 59.65% for FY2026, the value FY2025's 0.596523 cut down, Part 15
C's P-1 recomputed from the stored figures. It tests one leg of W-1's
thesis, that capital spending does no lasting damage to margins; it cannot
test the cloud margin catching up, no segment figure being offered. The
reasons it cites, 1.2, 1.4, 1.7 and 1.8 on the CLI's run, are about where
revenue comes from and AI in the products; none says anything about a
margin, Item 1 saying nothing about one. Item 7, which does, is the
...
Logged 19 September (twenty-seventh session). The check asks that each
reading's accession, filed date and source, and each proposal's dates and
statement, reach the answer somewhere. Five wrong versions of the
rendering dropped one of them from its own line and passed the check,
since the threshold's line and the row print the same accession, date and
statement; `tests/test_thesis_formatter.py` holds each line whole since
4e2b472. The rendering also prints a refused section's reason as the code
raised it, "Item 7, claim 8: a digit in the claim; a figure appears only
...
trading day has closed since the last run: the session's first golden
run, at 03:07 UTC on the 19th, added VIX, TNX_10Y and IRX_3M for
2026-09-18, 206 to 209, which was predicted as no change and missed.

### JPMorgan's fixture carries no share count

**Trigger:** the next read of JPMorgan's company facts for any field.

Logged 17 September (twenty-second session). `edgar_facts_jpm.csv` was
written from a read of the artifact on 13 September and the artifact is
not stored, because the exclusion decides before the facts are fetched; it
was not read again for `shares_outstanding`, so the field is unresolved
for the bank in every year by construction and the test asserts that. It
says nothing about what JPMorgan files under the tag. Part 13 C's note of
17 September records the same.
...
**Trigger:** a filer reporting a year on a form outside D29's list.
...
**Trigger:** pending decision 10: the first question about a quarter.
