...
| `tests/golden/KNOWN_GAPS.md` | Open decisions, resolved decisions, and why obvious fixes are wrong. Every open entry carries what it blocks: **82 open, 16 a named Order, 66 nothing, 79 to carry** once the three resolved-in-body entries are discounted. Read the Order-4 ones. **New and worth reading whole: "Order 4, the filings reader: Parts 12 and 13, and what is built", and "PHI-3.2's code list fails open, and a code carries no date".** |
